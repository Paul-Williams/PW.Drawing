...
using static PW.Drawing.ImageHelper;
...
namespace PW.Drawing
{
  public static class BitmapExtensions
  {
    ...
    /// </summary>
    public static void SaveHQ(this Image bitmap, FilePath path) => bitmap.SaveHQ(path, 95L);

    /// <summary>
    /// Saves the image at high quality to the specified path with the specified 0-100 (low-high) quality.
    /// </summary>
    public static void SaveHQ(this Image image, FilePath path, long compression)
    {
      if (image is null) throw new ArgumentNullException(nameof(image));
      if (path is null) throw new ArgumentNullException(nameof(path));
      if (compression is < (long)CompressionBounds.Min or > (long)CompressionBounds.Max)
        throw new ArgumentOutOfRangeException(nameof(compression), "Value must be between 0 and 100 (inclusive)");
